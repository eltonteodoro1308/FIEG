...
Static Function fMontaPlan()
	...
	cQuery += " 		ISNULL(SC7.C7_EMISSAO, '') AS DATA_EDITAL, "
	cQuery += "         C7_NUMPR AS NUMERO_PROCESSO, "
	cQuery += " 	    CASE SC7.C7_NUMPR WHEN '' THEN 'LICITAÇÃO'  "
	cQuery += " 			ELSE  CASE SUBSTRING(SC7.C7_NUMPR, 0, 3)  "
	...
	cQuery += "  	   '' AS C1_XCODCOM, "
	cQuery += "  	   SC7.C7_QUJE, "
	cQuery += "  	   SC7.C7_QUANT, "
	cQuery += "  	   SC7.C7_PRECO, "
	cQuery += "  	   '' AS CD_FORN_SC, "
	...
	cQuery += " 		AND SC7.C7_FILIAL >= '" + MV_PAR01 + "'  "
	cQuery += "			AND SC7.C7_FILIAL <= '" + MV_PAR02 + "' "
	cQuery += " 		AND SC7.C7_EMISSAO >= '" + DTOS(MV_PAR03) + "'  "
	cQuery += " 		AND SC7.C7_EMISSAO <= '" + DTOS(MV_PAR04) + "' "
	cQuery += "         AND ( SC7.C7_NUMSC = '      ' OR SC7.C7_NUMSC = '' OR SC7.C7_NUMSC IS NULL ) "
	cQuery += " ORDER BY FILIAL_PC, NUMERO_PC, FILIAL_COT, NUMERO_COT, ITEM_COT, PRODUTO_PC, ITEM_PC "
	...
	While !(_cAlias)->(Eof())
		...
		(_cAlias)->ITEM_PC <> itemPcAnt
			...
			If (_cAlias)->B1_CONTA = '12030103' .or. (_cAlias)->B1_CONTA = '32010102003'
				...
				//ESTE CAMPO FOI ALTERADO PARA "??" POIS NÃO TEMOS COMO COMPARAR O VALOR PAGO(PC) COM O VALOR SOLICITADO(SC)
				//ESTE VALOR DEVERÁ SER PREENCHIDO MANUALMENTE
				ARRAY_DADOS[42] := "??" // PERCENTUAL DE EXECUÇÃO = VALOR PAGO / VALOR CONTRATADO
			ENDIF
			...
			// ESTE CAMPO FOI ALTERADO PARA "" POIS NÃO TEMOS A INFORMAÇÃO DA SC PARA COMPOR ESTE CAMPO
			ARRAY_DADOS[44] := ""
