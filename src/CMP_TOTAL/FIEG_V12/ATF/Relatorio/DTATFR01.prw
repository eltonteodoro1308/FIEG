...
/*/================================================================================================================================/*/
/*/{Protheus.doc} DTATFR01
Rotina para imprimir o relatorio demonstrativo de valores por grupo.
...
User Function DTATFR01

	Local lRet := .T.
	Local dDtLim := CTOD("")
	Local cSession := GetPrinterSession()
	Local cFilePrint := "DTATFR01_"+Dtos(MSDate())+StrTran(Time(),":","")+".PDF"
	Local oSetup := nil
	Local xPathPDF := AllTrim(GetTempPath())
	Local xPatherver := MsDocPath()
	Private oReport
	Private cPerg := "DTATFR01"
	Private xDtHrEmis := ""
	Private xNomeUsr := ""
	Private xNomeGrp := ""
	Private nTmEntid := 0
	...
	*/
	...
	If ( !oReport:lInJob )
		oSetup := FWPrintSetup():New(nFlags, "Demonstrativo de valores por grupo")
...
Static Function ReportDef(oReport,oSetup,cFilePrint)
	...
	BeginSql Alias cAliasQry
		SELECT
		LEFT(N1_FILIAL,%Exp:nTmEntid%) AS N3GRUPO,
		N3_CCONTAB,
		COALESCE(CT1_DESC01,%Exp:cSpace1%) AS CT1_DESC01,
		...
		N1_QUANTD > 0 AND
		N1_STATUS <> '0'
		GROUP BY LEFT(N1_FILIAL,%Exp:nTmEntid%),N3_CCONTAB,COALESCE(CT1_DESC01,%Exp:cSpace1%)
		ORDER BY 1,2
	EndSql
	...
	If lSegue
		...
		While !(cAliasQry)->(EOF())
			...
			if (nLn+1)>46
				Aadd(aItens,{.t.,"1",xNomeGrp,"ITENS","VLR.COMPRA","DEPRECIAÇÃO","SALDO RESIDUAL","DEPREC. MES"})
				nLn := 1
				nPagTot++
			endif

			if cGrupoOld <> Alltrim((cAliasQry)->N3GRUPO)
				if !Empty(cGrupoOld)
				...
				cGrupoOld := Alltrim((cAliasQry)->N3GRUPO)
				SM0->(DbSeek(cEmpAnt+cGrupoOld))
				xNomeGrp := SM0->M0_NOMECOM
				aTotais := {0,0,0,0,0}
				if (nLn+1)>46
					nLn := 1
					nPagTot++
				endif
				Aadd(aItens,{nLn==1,"1",xNomeGrp,"ITENS","VLR.COMPRA","DEPRECIAÇÃO","SALDO RESIDUAL","DEPREC. MES"})
				nLn += 1
		...
		For n1:=1 to Len(aItens)
			...
			elseif aItens[n1,2] == "1"
				...
				nLRep -= 1
				oReport:Say(nLRep,NSTARTCOL+5,aItens[n1,3],oFont8N:oFont)
...
Static Function ImpMask(cLogo,oBrush,nPagTot,nPag,aItens,n1)
	...
	oReport:Say(030,NSTARTCOL+270,"QUADRO DEMONSTRATIVO DE VALORES - POR GRUPO", oFont9N:oFont)
